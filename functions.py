...
def exit_bank(Debug=False):
    ...
    x = random.randrange(631, 651)
    if Debug:
        print('x: ', x)
    y = random.randrange(49, 69)
    if Debug:
        print('y: ', y)
...
def find_Object_precise(item, left=0, top=0, right=0, bottom=0):
    ...
    if len(contours) != 0:
        ...
        # Define a maximum offset from the center (adjust this based on your preference)
        max_offset = 5

        # Generate random offsets within the range of [-max_offset, max_offset]
        offset_x = random.randint(-max_offset, max_offset)
        offset_y = random.randint(-max_offset, max_offset)

        # Add the offsets to the center coordinates to get the final x and y coordinates
        x = center_x + offset_x
        y = center_y + offset_y
...
def skill_lvl_up():
    ...
    screen_Image(0, 0, 971, 725)
    img_rgb = cv2.imread(r"image/screenshot.png")
...
def skill_lvl_up_new(lv_image):
    ...
    cv2.imwrite('images/matched_screen.png', img_rgb)
...
def pick_item(v, u):
    ...
    d = random.uniform(0.01, 0.15)
    b = random.uniform(0.1, 0.6)
    # print(f"picking up item at {x} {y}")
    pyautogui.moveTo(x, y, duration=b)
    time.sleep(d)
    pyautogui.click(button='left')
    time.sleep(c)
...
def Image_Rec_single(image, iheight=3, iwidth=3, threshold=0.7, clicker='left', ispace=10, playarea=True):
    global icoord
    global iflag
    if playarea:
        screen_Image(0, 0, 971, 800)
    else:
        screen_Image(0, 0, 971, 800)
...
def Image_Rec_single_2(image, iheight=3, iwidth=3, threshold=0.7, clicker='left', ispace=10, playarea=True):
    global icoord
    global iflag
    if playarea:
        screen_Image(0, 0, 700, 700)
    else:
        screen_Image(0, 0, 700, 700)
    img_rgb = cv2.imread('images/screenshot.png')
    # print('screenshot taken')
...
def Image_Rec_single_3(image, iheight=3, iwidth=3, threshold=0.7, clicker='left', ispace=10, playarea=True):
    global icoord
    global iflag
    if playarea:
        screen_Image(0, 0, 971, 800)
...
def invent_count(object, threshold=0.8):
    screen_Image(720, 388, 965, 719, 'inventshot2.png')
    counter = 0
    img_rgb = cv2.imread('images/inventshot2.png')
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    template = cv2.imread('images/' + object, 0)
    w, h = template.shape[::-1]
    res = cv2.matchTemplate(img_gray, template, cv2.TM_CCOEFF_NORMED)
